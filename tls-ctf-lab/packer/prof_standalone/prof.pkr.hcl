...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "ARCH=$(dpkg --print-architecture) && CODENAME=$(lsb_release -cs) && echo \"deb [arch=$ARCH signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu $CODENAME stable\" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null",
      "sudo apt-get update",
